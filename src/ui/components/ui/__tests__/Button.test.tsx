...
test("Button applies variant classes", () => {
  ...
  expect(Button({ children: "A", variant: "soft" }).props.className).toContain("border-gray-300");
  expect(Button({ children: "A", variant: "outline" }).props.className).toContain("border-gray-300");
  expect(Button({ children: "A", variant: "ghost" }).props.className).toContain("bg-transparent");
  expect(Button({ children: "A", variant: "destructive" }).props.className).toContain("text-[#b91c1c]");
});

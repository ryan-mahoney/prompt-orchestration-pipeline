...
export default function JobCard({
  ...
  return (
    <button type="button" className="rounded-xl border bg-white p-4 text-left shadow-sm" onClick={onClick}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-base font-semibold">{job.name}</div>
          <div className="text-sm text-slate-500">{job.pipelineLabel ?? pipeline?.name ?? job.pipeline ?? "—"}</div>
        </div>
      ...
      <div className="mt-3 text-sm text-slate-600">Cost: {formatCurrency4(job.totalCost ?? 0)} · Duration: {Math.max(0, Math.floor(overallElapsedMs / 1000))}s</div>
    </button>
  );
}

...
export default function JobTable({
  ...
  if (jobs.length === 0) {
    return <div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-8 text-center text-sm text-slate-500">No jobs to show here yet.</div>;
  }
        ...
        <tbody className="divide-y divide-slate-100">
          {jobs.map((job) => {
            ...
            return (
              <tr
                key={job.jobId}
                className={validId ? "cursor-pointer transition hover:bg-slate-50/50" : "cursor-not-allowed opacity-60"}
              ...
              >
                ...
                <td className="px-4 py-4 text-sm text-slate-700">{job.pipelineLabel ?? job.pipeline ?? pipeline?.name ?? "—"}</td>
                <td className="px-4 py-4">
                  <Badge intent={getIntent(job.status)}>{job.status}</Badge>
                </td>
                <td className="px-4 py-4 text-sm text-slate-700">{currentTask?.name ?? currentTaskId ?? "—"}</td>
                <td className="px-4 py-4">
                ...
                <td className="px-4 py-4 text-sm text-slate-700">{countCompleted(job)} of {job.taskCount}</td>
                <td className="px-4 py-4">
                  <div className="text-sm text-slate-700">{totalCost > 0 ? formatCurrency4(totalCost) : "—"}</div>
                  {totalTokens > 0 ? <div className="text-xs text-slate-500">{formatTokensCompact(totalTokens)}</div> : null}
                </td>
